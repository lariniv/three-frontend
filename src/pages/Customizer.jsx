      ...
      {!snap.intro && (
        <>
              ...
              <div className="editortabs-container">
                {EditorTabs.map((tab) => (
                  <Tab
                    key={tab.name}
                    tab={tab}
                    callbackFn={() => setActiveEditorTab(tab.name)}
                  />
                ))}
          ...
          >
            <CustomButton
              type="filled"
              callbackFn={() => (state.intro = true)}
              title="Go back"
              className={"rounded w-fit px-4 py-2.5 font-bold text-sm"}
            />
          </motion.div>
          ...
          >
            {FilterTabs.map((tab) => (
              <Tab
                ...
                callbackFn={() => handleActiveFilterTab(tab.name)}
              />
            ))}
            <Tab
